...
export function EditPostModal({ isOpen, onClose, onSuccess, post }: EditPostModalProps) {
  ...
  const [media, setMedia] = useState<any[]>(post.media && Array.isArray(post.media) ? post.media : [])
  ...
  useEffect(() => {
    if (post) {
      ...
      setTags(post.tags || [])
      setMedia(post.media && Array.isArray(post.media) ? post.media : [])
    }
  }, [post])
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Edit Post" size="lg">
        <div className="text-center py-8">
          <p className="text-dark-300 mb-4">You need to sign in to edit a post.</p>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData.error || 'Failed to update post'
        
        if (response.status === 401) {
          throw new Error('Please sign in to edit a post.')
